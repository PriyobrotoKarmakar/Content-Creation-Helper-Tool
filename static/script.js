    ...
    reader.onload = function(e) {
      userContent = `
        <div style="display: flex; flex-direction: column; gap: 10px;">
          <img src="${e.target.result}" alt="Uploaded" style="max-width: 200px; border-radius: 8px;">
          ${message ? `<p>${message}</p>` : ''}
        </div>
      `;
    ...
    if (contentModeActive && data.content) {
      ...
      const contentHTML = `
        <div class="content-container">
          ...
          <div class="content-text" id="content-text-${chatId}" data-original-content="${encodeURIComponent(originalContent)}">${formattedContent}</div>
        </div>
      `;
      ...
      if (data.success && data.image) {
        // Display thumbnail with generated image
        const thumbnailHTML = `
            ...
            <div class="thumbnail-header">
              ...
              <div class="thumbnail-buttons">
                <button class="content-copy-btn" onclick="copyContent(${chatId}, false)">
                  <img src="static/images/copy-selected-icon.svg" class="copy-btn-icon" alt="Copy Prompt">
                  Copy Prompt
                </button>
                <button class="download-btn" onclick="downloadImage('${data.image}', 'thumbnail-${Date.now()}.png')">
                  <img src="static/images/download-icon.svg" class="copy-btn-icon" alt="Download">
                  Download
                </button>
              </div>
            </div>
            <div class="content-text prompt-text" id="content-text-${chatId}" data-original-content="${encodeURIComponent(originalPrompt)}">
              <b>Prompt used:</b><br>
              ${data.prompt.replace(/\n/g, "<br>")}
            </div>
            <div class="thumbnail-preview">
              <img src="data:image/png;base64,${data.image}" alt="Generated Thumbnail" class="thumbnail-image">
            </div>
          </div>
        `;
        ...
        const thumbnailHTML = `
          <div class="thumbnail-container">
            ...
            <div class="content-text" id="content-text-${chatId}" data-original-content="${encodeURIComponent(originalPrompt)}">
              ${data.prompt.replace(/\n/g, "<br>")}
            </div>
            <div class="thumbnail-message">${data.message || "Image generation failed. You can use this prompt with another tool."}</div>
          </div>
        `;
        aiChatArea.innerHTML = thumbnailHTML;
      }
...
function copyContent(chatId, isContent) {
  ...
  if (contentText.hasAttribute('data-original-content')) {
    ...
    textToCopy = decodeURIComponent(contentText.getAttribute('data-original-content'));
  } else {
  ...
  navigator.clipboard
    ...
    .then(() => {
      ...
      copyBtn.innerHTML = `<img src="static/images/copy-selected-icon.svg" class="copy-btn-icon" alt="Copy"> Copied!`;
      copyBtn.classList.add("copied");
      
      setTimeout(() => {
        copyBtn.innerHTML = `<img src="static/images/copy-selected-icon.svg" class="copy-btn-icon" alt="Copy"> Copy`;
